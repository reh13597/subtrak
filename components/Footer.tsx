...
import { Github, Mail, Shield, Heart } from "lucide-react";
import { useUserProfile } from "@/hooks/useUserProfile";

export default function Footer() {
  const { cognitoId, loading } = useUserProfile();
  const isAuthed = !loading && cognitoId !== null;

  const footerLinks = isAuthed
    ? [
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div className="col-span-1 md:col-span-1 space-y-6">
            <Link href={isAuthed ? "/dashboard" : "/"} className="flex items-center space-x-2 group">
